...
#define MIN_HEIGHT_CLOUD 3
#define MAX_HEIGHT_CLOUD 8
...
Cloud::Cloud(int x, int y, int z, Chunk *chunk) {
    ...
    int height_cloud = MIN_HEIGHT_CLOUD + this->random(MAX_HEIGHT_CLOUD - MIN_HEIGHT_CLOUD);
    ...
    for (int i = -radius_cloud; i < radius_cloud; ++i) {
        for (int j = -radius_cloud; j < radius_cloud; ++j) {
            for (int k = y + height_cloud - radius_cloud; k < y + height_cloud + radius_cloud; ++k) {
                if (z + j < 16 && x + i < 16 && x + i > -1 && z + j > -1) {
                    if (chunk->blocks[x + i][k][z + j] == AIR && x % 2 != 0) {
                        chunk->blocks[x + i][k][z + j] = CLOUD;
                    }
                }
            }
        }
